...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (char c:word.toCharArray()) {
			int indVowel = vowels.indexOf(c);
			if ((indVowel!=-1) & (lastIsVowel == 0)) {
				count += 1;
			}
			if (indVowel!=-1) {
				lastIsVowel = 1;
		...
		if (word.length() >= 2) {
			if(word.charAt(word.length()-1)=='e' & vowels.indexOf(word.charAt(word.length()-2)) == -1) {
				if  (count > 1) {
					count -= 1;
				}
			}
	...
	public double getFleschScore()
	{
		...
		double nSentences = (double) getNumSentences();
		double nSyllable = (double) getNumSyllables();
		
	    return 206.835 - 1.015 * nWord/ nSentences -84.6 *nSyllable/nWord ;
	}
