...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for (int pos=0;pos<=s.length();pos++) {
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				String temp = null;
				if (pos<s.length()) {
					temp = s.substring(0,pos) + Character.toString(c) + s.substring(pos);
				}
				else {
					temp = s + Character.toString(c);
				}
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		if (s.length()>0) {
			for (int i=0;i<s.length();i++) {
				...
				if(!currentList.contains(temp) & temp.length()>0) {
					if (wordsOnly) {
						if (dict.isWord(temp)) {
							currentList.add(temp);
						}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while (queue.size()>0 & retList.size()<numSuggestions) {
			...
			List<String> can = distanceOne(curr, true);
			for (String s:can) {
